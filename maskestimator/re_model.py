...
class NeuralMaskEstimation:
    ...
    def get_model(self,
                  ...
                  is_adapt=False):
        if is_stateful == True:
            input_sequence = layers.Input(shape=(self.truncate_grad, self.spec_dim * self.number_of_stack),
                                          batch_size=MAX_SEQUENCE)  # time step * feature_size
        else:
            input_sequence = layers.Input(
                shape=(self.truncate_grad, self.spec_dim * self.number_of_stack))  # time step * feature_size
        LSTM_layer = (LSTM(self.spec_dim,
                           ...
                           go_backwards=False,
                           unroll=True,
                           recurrent_initializer=initializers.RandomUniform(minval=-self.recurrent_init,
                                                                            maxval=self.recurrent_init),  # 0.04
                           name='lstm'
                           ))(input_sequence)
        ...
        if is_adapt == False:
            model.compile(
                ...
                optimizer=optimizers.RMSprop(lr=self.lr, epsilon=1e-06, clipnorm=1.0))
        else:
            model.compile(
                loss={'speech_mask': 'binary_crossentropy', 'noise_mask': 'binary_crossentropy'},
                metrics=['acc'],
