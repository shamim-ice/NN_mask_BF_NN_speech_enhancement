...
class Shape_data:
    ...
    def convert_for_train(self, data, label1, label2):
        ...
        number_of_sample = np.int((number_of_frames - (self.left_stack + self.right_stack + 1)) / (
                    self.number_of_skip_frames + 1))  # # number of sample
    ...
    def get_padding_features(self, predict_features):
        # print('padding',predict_features.shape)
    ...
    def get_padding_features_lstm(self, predict_features):
        # print('padding',predict_features.shape)
        batch, sequence, feature_order = np.shape(predict_features)
        padding_feature = np.zeros((MAX_SEQUENCE, sequence, feature_order), dtype=np.float32)
        for i in range(batch, MAX_SEQUENCE + 1, batch):
            # print(i)
            padding_feature[i - batch: i, :, :] = predict_features
        return padding_feature, batch
