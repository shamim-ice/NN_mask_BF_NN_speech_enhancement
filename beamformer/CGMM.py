...
class complexGMM_mvdr:
    ...
    def get_spatial_correlation_matrix(self, speech_data):
        ...
        complex_spectrum, _ = util.get_3dim_spectrum_from_data(speech_data, self.fft_length, self.fft_shift,
                                                               self.fft_length)
        #print('complex spectrum',complex_spectrum.shape,complex_spectrum)
        ...
        for ite in range(0, self.number_of_EM_iterate):
            ...
            for f in range(0, number_of_bins):
                ...
                if np.linalg.cond(R_noisy_onbin,p=None) < self.condition_number_inv_threshold:
                    R_noisy_onbin = R_noisy_onbin + self.condition_number_inv_threshold * np.eye(
                        number_of_channels) * np.max(np.diag(R_noisy_onbin))
                if np.linalg.cond(R_noise_onbin) < self.condition_number_inv_threshold:
                    R_noise_onbin = R_noise_onbin + self.condition_number_inv_threshold * np.eye(
                        number_of_channels) * np.max(np.diag(R_noise_onbin))
                ...
                for t in range(0, number_of_frames):
                    ...
                    # update phi (real)
                    phi_noise[t, f] = np.real(
                        np.trace(np.matmul(corre, R_noise_inv), dtype=np.float64) / number_of_channels)
                    phi_noisy[t, f] = np.real(
                        np.trace(np.matmul(corre, R_noisy_inv), dtype=np.float64) / number_of_channels)
                    if phi_noise[t, f] == 0:
    ...
    def apply_beamformer(self, beamformer, complex_spectrum):
        ...
        return util.spec2wav(enhanced_spectrum, self.sampling_frequency, self.fft_length, self.fft_length,
                             self.fft_shift)